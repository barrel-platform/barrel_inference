...
%%% which routes load attempts to per-model barrel_inference_server_loader
%%% workers so the config server itself never blocks on a model load.

-module(barrel_inference_server_config).
...
max_request_body_bytes() ->
    persistent_term:get({?MODULE, max_request_body_bytes}, 268435456).
...
%% `{barrel_inference_load_progress, ModelId}` lines to the client. Returns
%% `ok` synchronously after subscribing `Caller`; the caller then
%% receives:
%%
%%   {barrel_inference_load_progress, ModelId}              every 2 s while loading
...
%% (see `barrel_inference_server_pipeline:wait_for_load/3'). The call itself
...
init([]) ->
    ...
    persistent_term:put(
        ...
        app_env(max_request_body_bytes, 268435456)
    ),
    persistent_term:put(
...
handle_call({ensure_loaded_async, ModelId, Caller}, _From, S = #state{load_policy = Policy}) ->
    case fast_check(ModelId) of
        ready ->
            Caller ! {barrel_inference_load_done, ModelId, ok},
            {reply, ok, S};
        not_ready when Policy =:= preloaded ->
            {reply, {error, not_preloaded}, S};
        not_ready when Policy =:= reject ->
...
await_loader(ModelId, From, Deadline, S = #state{loaders = L}) ->
    case maps:find(ModelId, L) of
        {ok, {Pid, _Mon}} ->
            barrel_inference_server_loader:await(Pid, From, Deadline),
            S;
        error ->
            {ok, Pid} = barrel_inference_server_loaders_sup:start_loader(ModelId),
...
subscribe_loader(ModelId, Caller, S = #state{loaders = L}) ->
    case maps:find(ModelId, L) of
        {ok, {Pid, _Mon}} ->
            barrel_inference_server_loader:subscribe(Pid, Caller),
            S;
        error ->
            {ok, Pid} = barrel_inference_server_loaders_sup:start_loader(ModelId),
            Mon = monitor(process, Pid),
