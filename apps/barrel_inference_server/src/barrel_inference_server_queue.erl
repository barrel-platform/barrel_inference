...
%%% waiters. It never calls barrel_inference and never sees barrel_inference messages.
...
-module(barrel_inference_server_queue).
...
-record(state, {
    model :: binary(),
    policy :: barrel_inference_server_config:pool_policy() | undefined,
...
start_link(ModelId) when is_binary(ModelId) ->
    Name = {via, barrel_inference_server_registry, {queue, ModelId}},
    gen_server:start_link(Name, ?MODULE, [ModelId], []).

-spec acquire(binary(), pos_integer()) -> {ok, slot()} | {error, atom()}.
acquire(ModelId, TimeoutMs) ->
    {ok, Pid} = barrel_inference_server_queues_sup:ensure_queue(ModelId),
...
release(ModelId, Slot) when is_reference(Slot) ->
    case barrel_inference_server_registry:whereis_name({queue, ModelId}) of
        undefined -> ok;
    ...
    case barrel_inference_server_registry:whereis_name({queue, ModelId}) of
        undefined ->
            {Concurrency, _Depth, _Timeout} = policy_to_params(
                barrel_inference_server_config:pool_policy_for(ModelId)
            ),
...
init([ModelId]) ->
    ...
    Policy = barrel_inference_server_config:pool_policy_for(ModelId),
...
handle_call({acquire, HandlerPid, ReqTimeout}, From, S) ->
    ...
    if
        ...
        Depth >= S#state.depth_max ->
            barrel_inference_server_metrics:inc_queue_dropped(S#state.model, full),
            {reply, {error, pool_exhausted}, S};
        true ->
            WaiterRef = make_ref(),
            ...
            barrel_inference_server_metrics:set_queue_depth(
                S#state.model,
...
handle_info({waiter_timeout, WaiterRef}, S) ->
    case take_waiter(WaiterRef, S#state.waiters) of
        {ok, W, Q1} ->
            ...
            barrel_inference_server_metrics:inc_queue_dropped(S#state.model, timeout),
            barrel_inference_server_metrics:set_queue_depth(S#state.model, queue:len(Q1)),
