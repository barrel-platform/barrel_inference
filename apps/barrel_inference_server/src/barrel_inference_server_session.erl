...
%%%
%%% barrel_inference 0.5 pins the underlying seq_id to whatever opaque term we
...
%%%      `#barrel_inference_request.user_id'). Claude Code sends a stable
...
-module(barrel_inference_server_session).
...
-spec derive(cowboy_req:req(), #barrel_inference_request{}) -> binary().
derive(Req, R) ->
...
fallback_session(#barrel_inference_request{user_id = UserId}) when
...
prefix_hash(#barrel_inference_request{model_id = Model, messages = Messages}) ->
