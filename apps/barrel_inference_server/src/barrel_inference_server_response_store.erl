...
-module(barrel_inference_server_response_store).
...
handle_cast({put, ResponseId, Model, Messages}, S) ->
    ExpiresAt = now_ms() + barrel_inference_server_config:responses_store_ttl_ms(),
    Row = #row{
...
schedule_gc() ->
    Interval = barrel_inference_server_config:responses_store_gc_interval_ms(),
