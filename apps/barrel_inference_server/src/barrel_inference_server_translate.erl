...
%%% the server's internal `#barrel_inference_request{}` record.
%%%
%%% Two directions:
%%%
%%%   *_to_internal/1   - request body decoded by `json:decode/1`
%%%                       (a map) -> `{ok, #barrel_inference_request{}}` |
%%%                       `{error, Reason}`.
%%%   internal_to_*/2   - `#barrel_inference_request{}` plus the per-response
%%%                       state needed by the handler ->
%%%                       JSON-encodable map (or list, for SSE).
%%%
%%% This module is pure Erlang. No barrel_inference, no Cowboy, no I/O. Tests
%%% drive it directly with map literals.

-module(barrel_inference_server_translate).
...
-spec openai_chat_to_internal(map()) ->
    {ok, #barrel_inference_request{}} | {error, term()}.
openai_chat_to_internal(Body) when is_map(Body) ->
    try
        ...
        {ok, Base#barrel_inference_request{
            model_id = Model,
            messages = Messages,
            prompt = undefined,
            system = System,
...
openai_responses_to_internal(Body) when is_map(Body) ->
    try
        ...
        Base = base_request(Body, openai),
        Base1 = Base#barrel_inference_request{
            max_tokens = responses_max_tokens(Body, Base#barrel_inference_request.max_tokens),
            user_id = parse_metadata_user_id(Body)
        },
        {ok, Base1#barrel_inference_request{
            model_id = Model,
            messages = MessagesIn,
            prompt = undefined,
            system = System,
...
-spec classify_builtin(binary()) ->
    {server_tool, tool(), barrel_inference_server_tool_executor:spec()} | drop.
classify_builtin(Type) when is_binary(Type) ->
    case barrel_inference_server_tool_executor:lookup_type(Type) of
        {ok, Spec} ->
            {server_tool, barrel_inference_server_tool_executor:declare(Spec), Spec};
        not_found ->
            logger:info(#{event => builtin_tool_dropped, type => Type}),
            drop
    end.

%% Append operator-configured MCP server tools (the `barrel_inference_server_mcp'
...
maybe_inject_mcp(_ToolChoice, {Tools, ServerTools}) ->
    case barrel_inference_server_mcp:catalog() of
        {[], _} ->
            {Tools, ServerTools};
...
%% them based on barrel_inference's `cache_hit_kind`; the same coarse mapping
...
%% =============================================================================
%% Ollama: request -> #barrel_inference_request{}
%% =============================================================================

%% Translate `POST /api/generate` body into the internal request.
%% Empty `prompt` -> `is_preload = true`.
-spec ollama_generate_to_internal(map()) -> {ok, #barrel_inference_request{}} | {error, term()}.
...
-spec ollama_chat_to_internal(map()) -> {ok, #barrel_inference_request{}} | {error, term()}.
...
check_messages_cap(L) when is_list(L) ->
    Cap = barrel_inference_server_config:max_messages(),
...
check_tools_cap(L) when is_list(L) ->
    Cap = barrel_inference_server_config:max_tools(),
    case length(L) > Cap of
...
anthropic_stop_reason(Stats) ->
    case maps:get(finish_reason, Stats, stop) of
        stop ->
            %% barrel_inference 0.3.0 reports the matched stop string in
...
usage_map(Stats) ->
    ...
    %% Stats `cache_hit_kind` flag barrel_inference exposes so SDKs that
...
%% barrel_inference 0.4.0 surfaces accurate per-request cache token deltas
