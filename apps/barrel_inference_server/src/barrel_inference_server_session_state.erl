...
%%% continuation path.
%%%
%%% barrel_inference 0.8.0 reports the exact generated token ids in each
...
%%% routing the new suffix through `barrel_inference:continue/3' with
...
%%% Set on `barrel_inference_done' (only when the assembled length agrees with
...
-module(barrel_inference_server_session_state).
