%%% Per-model loader. Owns the (synchronous) barrel_inference:load_model/2 call
...
%%%
%%% The actual barrel_inference:load_model/2 call runs in a spawn_monitor'd
...
%%%
%%%   {barrel_inference_load_progress, ModelId}              periodic, every 2 s
%%%   {barrel_inference_load_done, ModelId, ok}              on success
%%%   {barrel_inference_load_done, ModelId, {error, Reason}} on failure

-module(barrel_inference_server_loader).
...
-record(state, {
    ...
    %% Pids that receive {barrel_inference_load_progress|done, ModelId, ...}
    subscribers :: [pid()],
    tick_timer :: undefined | reference(),
    worker :: undefined | {pid(), reference()},
    load_started_at :: undefined | integer(),
    %% Monitor on the underlying barrel_inference_model gen_statem once it is
...
handle_cast({subscribe, Pid}, S = #state{state = loaded, model_id = Id}) ->
    case model_alive(Id) of
        true ->
            Pid ! {barrel_inference_load_done, Id, ok},
            {noreply, S};
        false ->
            Pid ! {barrel_inference_load_done, Id, {error, not_loaded}},
            {stop, normal, S}
    end;
handle_cast({subscribe, Pid}, S = #state{state = {failed, Reason}, model_id = Id}) ->
    Pid ! {barrel_inference_load_done, Id, {error, Reason}},
    {noreply, S};
...
handle_info(start_load, S = #state{model_id = ModelId}) ->
    ...
    %% manifest read). The slow part (barrel_inference:load_model/2) runs in
...
handle_info({progress_tick, _Ref}, S = #state{state = loading}) ->
    ...
    _ = [Pid ! {barrel_inference_load_progress, Id} || Pid <- Subs],
...
do_load(ModelId, Opts) ->
    try barrel_inference:load_model(ModelId, Opts) of
        ...
        {error, already_loaded} ->
            ...
            _ = barrel_inference_registry:unregister_name(ModelId),
            try barrel_inference:load_model(ModelId, Opts) of
                {ok, _} -> ok;
                {error, R} -> {error, R}
            catch
...
finalize(S = #state{model_id = Id, awaiters = A, subscribers = Subs, worker = W}, Result) ->
    ...
    _ = [Pid ! {barrel_inference_load_done, Id, Reply} || Pid <- Subs],
    NextState =
        case Result of
            ok -> loaded;
            {error, R} -> {failed, R}
...
%% with {noproc, {barrel_inference_model, not_found, _}}.
...
%% Resolve a model id into the barrel_inference:load_model/2 config map by
...
handle_missing(ModelId) ->
    case barrel_inference_server_config:auto_pull() of
        true ->
            case barrel_inference_server_models:pull(ModelId) of
...
%% Map a manifest into the option set barrel_inference:load_model/2 expects.
...
manifest_to_config(Manifest) ->
    ...
    %% barrel_inference_model_llama reads `context_opts` (forwarded to
    %% barrel_inference_nif:new_context/2) and `model_opts` (forwarded to
    %% barrel_inference_nif:load_model/2). Without these the NIF falls back to
...
%% Pre-Bucket-A workaround: even with upstream barrel_inference_cache_key now
%% covering every llama.cpp ftype value, this mapping keeps responses
%% stable across barrel_inference versions and short-circuits any future
