# Prompt: harden the Barrel Inference NIF against SIGSEGV

Drop this verbatim into a fresh Claude Code session pointed at the
sibling `barrel_inference` repo (`/Users/benoitc/Projects/barrel_inference`).

---

# Harden the Barrel Inference NIF against SIGSEGV

## Context

The downstream `barrel_inference_server` HTTP front end hit several BEAM-killing
...
`barrel_inference_nif` should refuse bad inputs with a clean `{error, _}`
...
5. `llama_load_model_from_file` aborts via `GGML_ASSERT` on certain
   malformed GGUFs. Today Barrel Inference dies; should return
   `{error, malformed_gguf}` via the abort callback already wired
   (`set_abort_callback: call`).

## Files

- `c_src/barrel_inference_nif.c` — the NIF surface. Most edits land here.
- `c_src/barrel_inference_safe.cpp` — C++ wrappers that catch llama.cpp
  exceptions and translate to integer return codes. The pattern
  to extend.
- `src/barrel_inference_nif.erl` — pure-Erlang side. No edits expected
  unless a new error atom needs registering.
- `test/barrel_inference_nif_tests.erl` (or eunit equivalent) — add the
  guard tests here.

The existing pattern is: every llama.cpp entry point is wrapped in
an `barrel_inference_safe_*` function in `barrel_inference_safe.cpp` declared
...
### 1. `barrel_inference_safe_n_ctx` / `barrel_inference_safe_n_batch` accessors

Add to `c_src/barrel_inference_safe.cpp`:
...
Add the matching `extern` declarations in `barrel_inference_nif.c` alongside
the existing `barrel_inference_safe_vocab_n_tokens` block.

### 2. Guard `nif_prefill`

After the vocab-range loop (around `barrel_inference_nif.c:1057-1063`),
...
`barrel_inference_safe_model_load_from_file` can return NULL + sentinel
...
prefill_returns_context_overflow_when_prompt_too_long_test() ->
    ...
    ?assertEqual({error, context_overflow},
                 barrel_inference_nif:prefill(Ctx, Tokens)).
...
2> {ok, M} = barrel_inference:load_model(<<"t">>,
2>     #{model_path => "...", context_size => 64, ...}).
3> barrel_inference:tokenize(M, binary:copy(<<"x ">>, 200)).
%% expected: {ok, [...200 tokens...]}
4> barrel_inference:complete(M, binary:copy(<<"x ">>, 200), #{}).
...
- Subprocess isolation (Ollama-style runner). The downstream
  ...
  in-process KV cache that's central to Barrel Inference's design.
