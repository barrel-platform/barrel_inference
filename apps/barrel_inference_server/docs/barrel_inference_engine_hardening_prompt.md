# Prompt: harden the Barrel Inference engine against decode wedges
...
`barrel_inference_nif_hardening_prompt.md` (which covers SIGSEGV / input
...
# Harden the Barrel Inference engine against decode wedges

## Context

The downstream `barrel_inference_server` runs an agentic tool "continue-loop"
...
items below are what's left, in priority order. Observed on Barrel Inference
...
mis-slices into garbage). `barrel_inference_server`'s `AGENTS.md` already notes
"byte-exact splice awaits an engine-side ask."

**Asks (either suffices):**
- Surface the exact generated token ids for a turn (in `barrel_inference_done`
  Stats, or via the existing `{barrel_inference_token_id, _, _}` stream).
