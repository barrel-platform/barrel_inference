%%% Tests for barrel_inference_server_translate. Pure module: no barrel_inference, no
%%% Cowboy, no I/O. We drive it with map literals.
-module(barrel_inference_server_translate_SUITE).
...
chat_minimal(_Cfg) ->
    ...
    ?assertEqual(<<"gpt-4o">>, R#barrel_inference_request.model_id),
    ?assertEqual(openai, R#barrel_inference_request.api),
    ?assertEqual(
        [#{role => <<"user">>, content => <<"hi">>}],
        R#barrel_inference_request.messages
    ),
    ?assertEqual(undefined, R#barrel_inference_request.system),
    ?assertEqual(false, R#barrel_inference_request.stream),
    ?assertEqual(1024, R#barrel_inference_request.max_tokens).
...
chat_extracts_system(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
    ?assertEqual(<<"sys1\n\nsys2">>, R#barrel_inference_request.system),
...
chat_temperature_default(_Cfg) ->
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(base_chat()),
    ?assertEqual(1.0, R#barrel_inference_request.temperature).

chat_seed_optional(_Cfg) ->
    {ok, R1} = barrel_inference_server_translate:openai_chat_to_internal(base_chat()),
    {ok, R2} = barrel_inference_server_translate:openai_chat_to_internal(
        (base_chat())#{<<"seed">> => 42}
    ),
    ?assertEqual(undefined, R1#barrel_inference_request.seed),
    ?assertEqual(42, R2#barrel_inference_request.seed).

chat_stop_string_or_array(_Cfg) ->
    {ok, R1} = barrel_inference_server_translate:openai_chat_to_internal(
        (base_chat())#{<<"stop">> => <<"END">>}
    ),
    {ok, R2} = barrel_inference_server_translate:openai_chat_to_internal(
        (base_chat())#{<<"stop">> => [<<"a">>, <<"b">>]}
    ),
    ?assertEqual([<<"END">>], R1#barrel_inference_request.stop),
    ?assertEqual([<<"a">>, <<"b">>], R2#barrel_inference_request.stop).

chat_tool_choice_required(_Cfg) ->
    Body = (base_chat())#{<<"tool_choice">> => <<"required">>},
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
    ?assertEqual(required, R#barrel_inference_request.tool_choice).
...
chat_tool_choice_named(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
    ?assertEqual({named, <<"search">>}, R#barrel_inference_request.tool_choice).

chat_tool_choice_none(_Cfg) ->
    Body = (base_chat())#{<<"tool_choice">> => <<"none">>},
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
    ?assertEqual(none, R#barrel_inference_request.tool_choice).
...
chat_tools_normalise(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
    [Tool] = R#barrel_inference_request.tools,
...
chat_invalid_missing_model(_Cfg) ->
    ...
    ?assertMatch(
        ...
        barrel_inference_server_translate:openai_chat_to_internal(Body)
    ).

chat_invalid_missing_messages(_Cfg) ->
    ?assertMatch(
        {error, {missing_field, <<"messages">>}},
        barrel_inference_server_translate:openai_chat_to_internal(
            #{<<"model">> => <<"x">>}
        )
    ).
...
completion_minimal(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_completion_to_internal(Body),
    ?assertEqual(<<"x">>, R#barrel_inference_request.model_id),
    ?assertEqual(<<"hi">>, R#barrel_inference_request.prompt),
    ?assertEqual([], R#barrel_inference_request.messages),
    ?assertEqual(none, R#barrel_inference_request.tool_choice).

completion_missing_prompt(_Cfg) ->
    ?assertMatch(
        {error, {missing_field, <<"prompt">>}},
        barrel_inference_server_translate:openai_completion_to_internal(
            #{<<"model">> => <<"x">>}
        )
    ).
...
embeddings_string_input(_Cfg) ->
    {ok, R} = barrel_inference_server_translate:openai_embeddings_to_internal(
        #{<<"model">> => <<"e">>, <<"input">> => <<"hello">>}
    ),
...
embeddings_token_array_rejected(_Cfg) ->
    ?assertMatch(
        ...
        barrel_inference_server_translate:openai_embeddings_to_internal(
            #{<<"model">> => <<"e">>, <<"input">> => [1, 2, 3]}
        )
    ).
...
anthropic_minimal(_Cfg) ->
    ...
    ?assertEqual(anthropic, R#barrel_inference_request.api),
    ?assertEqual(<<"claude">>, R#barrel_inference_request.model_id),
    ?assertEqual(undefined, R#barrel_inference_request.system).
...
anthropic_system_string(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual(<<"be helpful">>, R#barrel_inference_request.system).
...
anthropic_system_blocks(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual(<<"part1\n\npart2">>, R#barrel_inference_request.system).
...
anthropic_output_config_json_schema(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual({json_schema, Schema}, R#barrel_inference_request.response_format).
...
anthropic_output_config_absent_or_bad(_Cfg) ->
    ...
    {ok, R0} = barrel_inference_server_translate:anthropic_messages_to_internal(Body0),
    ?assertEqual(text, R0#barrel_inference_request.response_format),
    Body1 = Body0#{<<"output_config">> => #{<<"json_schema">> => <<"not a map">>}},
    {ok, R1} = barrel_inference_server_translate:anthropic_messages_to_internal(Body1),
    ?assertEqual(text, R1#barrel_inference_request.response_format).
...
anthropic_betas_body_parsed(_Cfg) ->
    ?assertEqual(
        [<<"a">>, <<"b">>],
        barrel_inference_server_translate:parse_anthropic_betas_body(
            #{<<"betas">> => [<<"a">>, <<"b">>]}
        )
    ),
    ?assertEqual(
...
anthropic_thinking_enabled(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual(enabled, R#barrel_inference_request.thinking),
    ?assertEqual(visible, R#barrel_inference_request.thinking_display),
    ?assertEqual(undefined, R#barrel_inference_request.thinking_budget).
...
anthropic_thinking_display_and_budget(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual(enabled, R#barrel_inference_request.thinking),
    ?assertEqual(omitted, R#barrel_inference_request.thinking_display),
    ?assertEqual(512, R#barrel_inference_request.thinking_budget).
...
anthropic_stop_sequences_parsed(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual([<<"\n\nHuman:">>, <<"END">>], R#barrel_inference_request.stop).
...
anthropic_metadata_user_id_captured(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual(<<"u-42">>, R#barrel_inference_request.user_id).
...
anthropic_metadata_user_id_absent_or_bad(_Cfg) ->
    ...
    {ok, R0} = barrel_inference_server_translate:anthropic_messages_to_internal(Body0),
    ?assertEqual(undefined, R0#barrel_inference_request.user_id),
    Body1 = Body0#{<<"metadata">> => #{<<"user_id">> => 42}},
    {ok, R1} = barrel_inference_server_translate:anthropic_messages_to_internal(Body1),
    ?assertEqual(undefined, R1#barrel_inference_request.user_id).
...
anthropic_content_blocks_tool_result(_Cfg) ->
    ...
    {ok, RA} = barrel_inference_server_translate:anthropic_messages_to_internal(BodyA),
    [#{content := CA}] = RA#barrel_inference_request.messages,
    ...
    {ok, RB} = barrel_inference_server_translate:anthropic_messages_to_internal(BodyB),
    [#{content := CB}] = RB#barrel_inference_request.messages,
...
anthropic_content_blocks_tool_result_is_error(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    [#{content := C}] = R#barrel_inference_request.messages,
...
anthropic_cache_control_captured_on_system(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    Hints = R#barrel_inference_request.cache_hints,
...
anthropic_cache_control_captured_on_tool(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertMatch([#{kind := tool}], R#barrel_inference_request.cache_hints).
...
anthropic_cache_control_captured_on_message_block(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertMatch([#{kind := message}], R#barrel_inference_request.cache_hints).
...
anthropic_no_cache_hints_when_unmarked(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertEqual([], R#barrel_inference_request.cache_hints).
...
anthropic_cache_hints_hash_is_stable(_Cfg) ->
    ...
    {ok, RA} = barrel_inference_server_translate:anthropic_messages_to_internal(BodyA),
    {ok, RB} = barrel_inference_server_translate:anthropic_messages_to_internal(BodyB),
    [#{hash := HashA}] = RA#barrel_inference_request.cache_hints,
    [#{hash := HashB}] = RB#barrel_inference_request.cache_hints,
    ?assertEqual(HashA, HashB).

%% usage.cache_read_input_tokens and cache_creation_input_tokens come
%% straight from Stats.cache_delta = #{read, created} in barrel_inference 0.4.0.
...
anthropic_usage_emits_cache_read_on_exact_hit(_Cfg) ->
    ...
    Resp = barrel_inference_server_translate:internal_to_anthropic_messages_response(
...
openai_usage_emits_cached_tokens_on_exact_hit(_Cfg) ->
    ...
    Resp = barrel_inference_server_translate:internal_to_openai_chat_response(
        <<"x">>, Stats, <<"m">>
    ),
    Usage = maps:get(<<"usage">>, Resp),
...
openai_chat_response_shape(_Cfg) ->
    ...
    R = barrel_inference_server_translate:internal_to_openai_chat_response(
        <<"hello">>, Stats, <<"gpt-4o">>
    ),
...
openai_chat_final_shape(_Cfg) ->
    ...
    Iolist = barrel_inference_server_translate:internal_to_openai_chat_final(
...
openai_completion_response_shape(_Cfg) ->
    R = barrel_inference_server_translate:internal_to_openai_completion_response(
        <<"out">>,
        #{
            prompt_tokens => 1,
            completion_tokens => 1,
...
openai_embedding_response_shape(_Cfg) ->
    R = barrel_inference_server_translate:internal_to_openai_embedding_response(
        [[1.0, 2.0], [3.0, 4.0]], 7, <<"e">>
    ),
...
anthropic_response_shape(_Cfg) ->
    ...
    R = barrel_inference_server_translate:internal_to_anthropic_messages_response(
...
%% barrel_inference 0.3.0 reports the matched caller-supplied stop string in
...
responses_string_input_normalises_to_single_user_message(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_responses_to_internal(Body),
    ?assertEqual(<<"gpt-4o">>, R#barrel_inference_request.model_id),
    ?assertEqual(openai, R#barrel_inference_request.api),
    ?assertEqual(
        [#{role => <<"user">>, content => <<"hello">>}],
...
responses_instructions_prepends_to_system(_Cfg) ->
    ...
    ?assertEqual(
        ...
        R#barrel_inference_request.system
    ).
...
responses_builtin_tool_dropped(_Cfg) ->
    ...
    persistent_term:erase({barrel_inference_server_config, builtin_tool_executors}),
    Body = #{
        <<"model">> => <<"gpt-4o">>,
        <<"input">> => <<"hi">>,
    ...
    {ok, R} = barrel_inference_server_translate:openai_responses_to_internal(Body),
    ?assertEqual([], R#barrel_inference_request.tools),
    ?assertEqual(#{}, R#barrel_inference_request.server_tools).
...
responses_namespace_tools_flattened(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_responses_to_internal(Body),
    Names = [maps:get(name, T) || T <- R#barrel_inference_request.tools],
...
responses_builtin_tool_with_executor_synthesised(_Cfg) ->
    ...
    persistent_term:put(
        {barrel_inference_server_config, builtin_tool_executors},
        #{
            <<"web_search">> => #{
                module => barrel_inference_server_tool_executor_stub,
                type => <<"web_search">>
            }
        }
    ),
    try
        ...
        {ok, R} = barrel_inference_server_translate:openai_responses_to_internal(Body),
        ?assertMatch([#{name := <<"web_search">>, schema := _}], R#barrel_inference_request.tools),
        ?assertMatch(
            #{<<"web_search">> := #{module := barrel_inference_server_tool_executor_stub}},
            R#barrel_inference_request.server_tools
        )
    after
        persistent_term:erase({barrel_inference_server_config, builtin_tool_executors})
    end.
...
chat_builtin_tool_dropped(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
    ?assertEqual([<<"f">>], [maps:get(name, T) || T <- R#barrel_inference_request.tools]),
    ?assertEqual(#{}, R#barrel_inference_request.server_tools).
...
chat_builtin_tool_with_executor_synthesised(_Cfg) ->
    with_stub_executor(fun() ->
        ...
        {ok, R} = barrel_inference_server_translate:openai_chat_to_internal(Body),
        ?assertMatch([#{name := <<"web_search">>}], R#barrel_inference_request.tools),
        ?assertMatch(
            #{<<"web_search">> := #{module := barrel_inference_server_tool_executor_stub}},
            R#barrel_inference_request.server_tools
        )
...
anthropic_custom_tool_kept(_Cfg) ->
    ...
    {ok, R} = barrel_inference_server_translate:anthropic_messages_to_internal(Body),
    ?assertMatch([#{name := <<"get_weather">>, schema := _}], R#barrel_inference_request.tools),
    ?assertEqual(#{}, R#barrel_inference_request.server_tools).
...
mcp_tools_injected_all_surfaces(_Cfg) ->
    with_mcp_catalog(fun() ->
        ChatBody = base_chat(),
        {ok, RC} = barrel_inference_server_translate:openai_chat_to_internal(ChatBody),
        assert_has_mcp_tool(RC),
        RespBody = #{<<"model">> => <<"gpt-4o">>, <<"input">> => <<"hi">>},
        {ok, RR} = barrel_inference_server_translate:openai_responses_to_internal(RespBody),
        ...
        {ok, RA} = barrel_inference_server_translate:anthropic_messages_to_internal(AnthBody),
...
assert_has_mcp_tool(R) ->
    ...
    ?assertMatch(
        #{<<"srv__ping">> := #{module := barrel_inference_server_tool_executor_mcp}},
        R#barrel_inference_request.server_tools
    ).
...
with_mcp_catalog(Fun) ->
    ...
    persistent_term:put({barrel_inference_server_mcp, catalog}, {Tools, ServerTools}),
    try
        Fun()
    after
        persistent_term:erase({barrel_inference_server_mcp, catalog})
    end.

stream_options_include_usage_parsed(_Cfg) ->
    Off = base_chat(),
    {ok, R0} = barrel_inference_server_translate:openai_chat_to_internal(Off),
    ?assertEqual(false, R0#barrel_inference_request.include_usage),
    On = (base_chat())#{
        <<"stream">> => true,
        <<"stream_options">> => #{<<"include_usage">> => true}
    },
    {ok, R1} = barrel_inference_server_translate:openai_chat_to_internal(On),
    ?assertEqual(true, R1#barrel_inference_request.include_usage).

parallel_tool_calls_parsed(_Cfg) ->
    {ok, RDefault} = barrel_inference_server_translate:openai_chat_to_internal(base_chat()),
    ?assertEqual(true, RDefault#barrel_inference_request.parallel_tool_calls),
    Off = (base_chat())#{<<"parallel_tool_calls">> => false},
    {ok, ROff} = barrel_inference_server_translate:openai_chat_to_internal(Off),
    ?assertEqual(false, ROff#barrel_inference_request.parallel_tool_calls).

chat_final_chunk_usage_null(_Cfg) ->
    Stats = #{prompt_tokens => 3, completion_tokens => 5, finish_reason => stop},
    Io = barrel_inference_server_translate:internal_to_openai_chat_final(
        Stats, <<"chatcmpl-1">>, <<"gpt-4o">>
    ),
...
usage_chunk_has_empty_choices_and_usage(_Cfg) ->
    ...
    Io = barrel_inference_server_translate:internal_to_openai_usage_chunk(
        Stats, <<"chatcmpl-1">>, <<"gpt-4o">>
    ),
    Decoded = json:decode(iolist_to_binary(Io)),
