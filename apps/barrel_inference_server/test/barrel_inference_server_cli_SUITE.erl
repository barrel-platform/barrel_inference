%%% End-to-end smoke for the barrel-inference escript.
%%%
...
%%% inference.
-module(barrel_inference_server_cli_SUITE).
...
help_prints_usage(Cfg) ->
    Out = run(Cfg, ["help"]),
    ?assert(string:str(Out, "barrel-inference pull") > 0),
    ?assert(string:str(Out, "barrel-inference list") > 0).
...
rm_removes_manifest(Cfg) ->
    ...
    ?assertEqual({error, not_found}, barrel_inference_server_models:get(<<"to-be-removed">>)).
...
make_tmp_dir() ->
    ...
    Dir = filename:join(
        Base,
        "barrel_inference_server_cli_" ++ integer_to_list(erlang:unique_integer([positive]))
    ),
