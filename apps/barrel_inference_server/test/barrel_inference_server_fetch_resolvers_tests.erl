...
%%
-module(barrel_inference_server_fetch_resolvers_tests).
...
parse_hf_simple_test() ->
    ?assertMatch(
        ...
        barrel_inference_server_fetch_resolvers:parse(<<"hf://org/repo/path/to/file.gguf">>)
    ).

parse_hf_revision_test() ->
    ?assertMatch(
        {ok, {hf, <<"org">>, <<"repo">>, <<"f.gguf">>, <<"v1.2">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"hf://org/repo/f.gguf@v1.2">>)
    ).

parse_hf_string_input_test() ->
    ?assertMatch(
        {ok, {hf, _, _, _, _}},
        barrel_inference_server_fetch_resolvers:parse("hf://a/b/c.gguf")
    ).

parse_hf_no_path_returns_undefined_path_test() ->
    ?assertMatch(
        {ok, {hf, <<"org">>, <<"repo">>, undefined, <<"main">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"hf://org/repo">>)
    ).

parse_hf_missing_repo_test() ->
    ?assertMatch(
        {error, {bad_hf_spec, _}},
        barrel_inference_server_fetch_resolvers:parse(<<"hf://org">>)
    ).

parse_ollama_default_tag_test() ->
    ?assertMatch(
        {ok, {ollama, <<"library">>, <<"llama3">>, <<"latest">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"ollama://library/llama3">>)
    ).

parse_ollama_with_tag_test() ->
    ?assertMatch(
        {ok, {ollama, <<"library">>, <<"llama3">>, <<"8b">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"ollama://library/llama3:8b">>)
    ).

parse_https_test() ->
    ?assertMatch(
        {ok, {http, <<"https://example.com/m.gguf">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"https://example.com/m.gguf">>)
    ).

parse_http_test() ->
    ?assertMatch(
        {ok, {http, <<"http://example.com/m.gguf">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"http://example.com/m.gguf">>)
    ).

parse_file_url_test() ->
    ?assertMatch(
        {ok, {file, <<"/abs/path">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"file:///abs/path">>)
    ).

parse_absolute_path_test() ->
    ?assertMatch(
        {ok, {file, <<"/srv/m.gguf">>}},
        barrel_inference_server_fetch_resolvers:parse(<<"/srv/m.gguf">>)
    ).

parse_unknown_scheme_test() ->
    ?assertMatch(
        {error, {unsupported_spec, _}},
        barrel_inference_server_fetch_resolvers:parse(<<"ftp://nope">>)
    ).
...
resolve_hf_url_shape_test() ->
    ...
    {ok, R} = barrel_inference_server_fetch_resolvers:resolve(Parsed),
    ?assertEqual(
...
resolve_http_test() ->
    {ok, R} = barrel_inference_server_fetch_resolvers:resolve({http, <<"https://e.com/m.gguf">>}),
    ?assertEqual(<<"https://e.com/m.gguf">>, maps:get(url, R)),
    ?assertEqual(<<"m.gguf">>, maps:get(out_basename, R)).

resolve_file_test() ->
    {ok, R} = barrel_inference_server_fetch_resolvers:resolve({file, <<"/srv/m.gguf">>}),
...
resolve_ollama_without_fetch_test() ->
    ?assertEqual(
        ...
        barrel_inference_server_fetch_resolvers:resolve(
...
resolve_ollama_manifest_test() ->
    ...
    {ok, R} = barrel_inference_server_fetch_resolvers:resolve(
...
hf_list_siblings_test() ->
    ...
    {ok, Files} = barrel_inference_server_fetch_resolvers:hf_list_siblings(
        <<"org">>, <<"repo">>, <<"main">>, Fetch
    ),
    ?assertEqual(
...
hf_list_siblings_404_test() ->
    ...
    ?assertMatch(
        ...
        barrel_inference_server_fetch_resolvers:hf_list_siblings(
            <<"o">>, <<"r">>, <<"main">>, Fetch
        )
    ).
...
hf_pick_gguf_prefers_q4_k_m_test() ->
    ...
    ?assertEqual(
        ...
        barrel_inference_server_fetch_resolvers:hf_pick_gguf(Files)
    ).

hf_pick_gguf_falls_back_to_first_test() ->
    Files = [<<"a.gguf">>, <<"b.gguf">>],
    ?assertMatch({ok, _}, barrel_inference_server_fetch_resolvers:hf_pick_gguf(Files)).

hf_pick_gguf_no_gguf_test() ->
    ?assertEqual(
        {error, no_gguf},
        barrel_inference_server_fetch_resolvers:hf_pick_gguf([<<"README.md">>, <<"config.json">>])
    ).
...
spec_hash_stable_test() ->
    ...
    H1 = barrel_inference_server_fetch_resolvers:spec_hash(P),
    H2 = barrel_inference_server_fetch_resolvers:spec_hash(P),
    ?assertEqual(H1, H2),
    ?assertEqual(16, byte_size(H1)).

spec_hash_unique_test() ->
    H1 = barrel_inference_server_fetch_resolvers:spec_hash(
        {hf, <<"a">>, <<"b">>, <<"c">>, <<"main">>}
    ),
    H2 = barrel_inference_server_fetch_resolvers:spec_hash(
        {hf, <<"a">>, <<"b">>, <<"d">>, <<"main">>}
    ),
