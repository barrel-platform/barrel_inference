%%% End-to-end CT suite against a real GGUF model. Mirrors barrel_inference's
%%% own `barrel_inference_real_model_SUITE` pattern: gated on `LLAMA_TEST_MODEL`
...
-module(barrel_inference_server_real_model_SUITE).
...
start_app(Path, Config) ->
    ...
    application:set_env(barrel_inference_server, prefill_timeout_ms, 120000),
    application:set_env(barrel_inference_server, generation_idle_timeout_ms, 60000),
    application:set_env(barrel_inference_server, max_total_ms, 240000),
    ...
    ModelId = <<"real">>,
    {ok, _} = barrel_inference:load_model(ModelId, model_config(Path, DiskSrv)),
    [
        {port, Port},
...
%% init_per_suite via barrel_inference:load_model/2, so if the load itself
...
load_and_decode_round_trip(Cfg) ->
    ...
    %% at the same instant.
    Info = barrel_inference:model_info(Model),
    ?assert(is_map(Info)),
    ...
    %% the test fast.
    {ok, Tokens} = barrel_inference:tokenize(Model, <<"hi">>),
    ...
    Outcome =
        receive
            {barrel_inference_token, Ref, _} -> token;
            {barrel_inference_done, Ref, _} -> done;
            {barrel_inference_error, Ref, Reason} -> {error, Reason}
        after 30000 ->
            timeout
        end,
...
drain_inference(Ref, BudgetMs) ->
    receive
        {barrel_inference_token, Ref, _} -> drain_inference(Ref, BudgetMs);
        {barrel_inference_done, Ref, _} -> ok;
        {barrel_inference_error, Ref, _} -> ok;
        {barrel_inference_cancelled, Ref} -> ok
    after BudgetMs ->
        ok
    end.
...
multi_turn_cache_delta_profile(Cfg) ->
    ...
    %% With barrel_inference 0.6.0's continue/3 wired in, turn 2 and turn 3
...
responses_server_tool_loop_runs(Cfg) ->
    ...
    persistent_term:put({barrel_inference_server_config, max_tool_iterations}, 2),
    try
        Url = ?config(base, Cfg) ++ "/v1/responses",
        Body = json:encode(#{
            <<"model">> => <<"real">>,
    ...
    after
        ...
        persistent_term:erase({barrel_inference_server_config, max_tool_iterations})
    end.
