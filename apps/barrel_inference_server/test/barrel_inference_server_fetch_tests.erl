...
%%
-module(barrel_inference_server_fetch_tests).
...
file_passthrough_returns_existing_path(#{doc := Doc}) ->
    ...
    ?assertEqual({ok, Path}, barrel_inference_server_fetch:fetch(list_to_binary(Path))).

file_passthrough_missing_returns_error(_Ctx) ->
    ?assertMatch(
        {error, {enoent, _}},
        barrel_inference_server_fetch:fetch(<<"/this/should/not/exist.gguf">>)
    ).

http_round_trip(#{port := Port, body := Body}) ->
    URL = url(Port, "/model.gguf"),
    {ok, Path} = barrel_inference_server_fetch:fetch(URL, #{force => true}),
    {ok, Got} = file:read_file(Path),
    ?assertEqual(Body, Got).

http_round_trip_caches(#{port := Port}) ->
    URL = url(Port, "/model.gguf"),
    {ok, P1} = barrel_inference_server_fetch:fetch(URL),
    {ok, P2} = barrel_inference_server_fetch:fetch(URL),
    ?assertEqual(P1, P2).
...
http_sha256_verify_pass(#{port := Port, sha := Sha}) ->
    ...
    Hex = bin_to_hex(Sha),
    ?assertMatch(
        {ok, _}, barrel_inference_server_fetch:fetch(URL, #{force => true, sha256 => Hex})
    ).
...
http_sha256_mismatch_returns_error(#{port := Port}) ->
    ...
    ?assertMatch(
        ...
        barrel_inference_server_fetch:fetch(URL, #{force => true, sha256 => Bogus})
    ).

http_progress_events_arrive(#{port := Port}) ->
    URL = url(Port, "/model.gguf"),
    Self = self(),
    {ok, _} = barrel_inference_server_fetch:fetch(URL, #{force => true, progress => Self}),
...
http_404_returns_error(#{port := Port}) ->
    ...
    ?assertMatch(
        ...
        barrel_inference_server_fetch:fetch(URL, #{force => true})
    ).
...
async_returns_jobref_and_done_message(#{port := Port}) ->
    URL = url(Port, "/model.gguf"),
    flush_inbox(),
    {ok, JobRef} = barrel_inference_server_fetch:fetch_async(URL, #{force => true}),
    ?assert(is_binary(JobRef)),
    receive
        {barrel_inference_fetch_done, JobRef, {ok, _Path}} -> ok
    after 5000 ->
        ?assert(false)
    end.
...
async_status_pending_then_done(#{port := Port}) ->
    ...
    case barrel_inference_server_fetch:fetch_status(JobRef) of
        {pending, #{phase := _}} -> ok;
        {done, _} -> ok;
        Other -> ?assertEqual({pending, '...'}, Other)
    end,
    receive
        {barrel_inference_fetch_done, JobRef, _} -> ok
    after 5000 -> ?assert(false)
    end,
    ?assertMatch({done, {ok, _}}, barrel_inference_server_fetch:fetch_status(JobRef)).

async_await_blocks_until_done(#{port := Port}) ->
    URL = url(Port, "/model.gguf"),
    flush_inbox(),
    {ok, JobRef} = barrel_inference_server_fetch:fetch_async(URL, #{force => true}),
    ?assertMatch({ok, _}, barrel_inference_server_fetch:fetch_await(JobRef, 5000)).
...
async_subscribe_extra_pid(#{port := Port}) ->
    ...
    Watcher = spawn(fun() ->
        receive
            {barrel_inference_fetch_done, _, R} -> Self ! {watcher, R}
        after 5000 -> Self ! {watcher, timeout}
        end
    end),
    {ok, JobRef} = barrel_inference_server_fetch:fetch_async(URL, #{force => true}),
    ok = barrel_inference_server_fetch:fetch_subscribe(JobRef, Watcher),
    receive
        {watcher, {ok, _}} -> ok;
...
received_any_progress(_Pid, 0) ->
    case
        receive
            {barrel_inference_fetch_progress, _, _, _} -> true
        after 0 -> false
        end
    of
        true -> true;
...
make_tmp_dir() ->
    ...
    Dir = filename:join(
        Base,
        "barrel_inference_server_fetch_tests_" ++ integer_to_list(erlang:unique_integer([positive]))
    ),
...
start_httpd(DocRoot) ->
    ...
    {ok, Pid} = inets:start(httpd, [
        {port, 0},
        {server_name, "barrel_inference-test"},
        {server_root, DocRoot},
