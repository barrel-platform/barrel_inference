...
%% tool, points the barrel_inference_server_mcp manager at it, and asserts the
%% manager publishes that tool into its catalog. No model needed.
-module(barrel_inference_server_mcp_SUITE).
...
end_per_suite(_Config) ->
    ...
    application:unset_env(barrel_inference_server, mcp_servers),
    ok.

end_per_testcase(_, _Config) ->
    catch gen_server:stop(barrel_inference_server_mcp, normal, 5000),
...
catalog_and_executor(Config) ->
    ...
    application:set_env(barrel_inference_server, mcp_servers, [
        #{id => <<"t">>, transport => {http, Url}}
    ]),
    {ok, _Pid} = barrel_inference_server_mcp:start_link(),
    ok = wait_client_ready(<<"t">>, 50),
    ok = barrel_inference_server_mcp:refresh(),
    {Tools, ServerTools} = barrel_inference_server_mcp:catalog(),
    ...
    ?assertMatch(
        #{module := barrel_inference_server_tool_executor_mcp, mcp_name := <<"t__echo">>}, Spec
    ),
    ...
    {ok, Result} = barrel_inference_server_tool_executor_mcp:execute(
        #{<<"text">> => <<"hello mcp">>}, Ctx
    ),
...
list_changed_refresh(Config) ->
    ...
    ok = barrel_inference_server_mcp:refresh(),
    {Tools0, _} = barrel_inference_server_mcp:catalog(),
...
resources_bridge(Config) ->
    ...
    {ok, ListResult} = barrel_inference_server_tool_executor_mcp:execute(
    ...
    {ok, ReadResult} = barrel_inference_server_tool_executor_mcp:execute(
        #{<<"uri">> => <<"mem://greeting">>},
    ...
    ?assertEqual(
        {error, missing_uri},
        barrel_inference_server_tool_executor_mcp:execute(
            #{}, ctx(maps:get(<<"r__read_resource">>, ServerTools))
        )
    ).
...
wait_catalog_tool(Name, N) ->
    {Tools, _} = barrel_inference_server_mcp:catalog(),
