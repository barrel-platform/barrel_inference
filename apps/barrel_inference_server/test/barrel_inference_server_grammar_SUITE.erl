...
no_tools_returns_empty(_Cfg) ->
    ?assertEqual({ok, <<>>}, barrel_inference_server_grammar:from_tools(undefined, auto)).

empty_tools_returns_empty(_Cfg) ->
    ?assertEqual({ok, <<>>}, barrel_inference_server_grammar:from_tools([], auto)).
...
tool_choice_none_returns_empty(_Cfg) ->
    ...
    ?assertEqual({ok, <<>>}, barrel_inference_server_grammar:from_tools(Tools, none)).
...
auto_emits_text_or_tool(_Cfg) ->
    ...
    {ok, G} = barrel_inference_server_grammar:from_tools(Tools, auto),
...
required_omits_text_branch(_Cfg) ->
    ...
    {ok, G} = barrel_inference_server_grammar:from_tools(Tools, required),
...
named_pins_to_tool(_Cfg) ->
    ...
    {ok, G} = barrel_inference_server_grammar:from_tools(Tools, {named, <<"book">>}),
    {RootRhs, _} = root_body(G),
