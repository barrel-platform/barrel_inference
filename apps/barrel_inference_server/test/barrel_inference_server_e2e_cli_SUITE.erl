...
%%% barrel-inference escript at `_build/<profile>/bin/barrel-inference` and the HTTP
%%% surface, so this is the closest we get to a real user session.
%%%
%%% The stub model backend (barrel_inference_model_stub) produces phash2-based
%%% deterministic tokens, so we can drive `run` and `/api/generate`
%%% without a real GGUF, while still exercising the full pipeline:
%%% pull -> manifest -> registry-aware loader -> barrel_inference:load_model
%%% -> streaming inference -> response framing.
-module(barrel_inference_server_e2e_cli_SUITE).
...
init_per_suite(Config) ->
    ...
    application:set_env(barrel_inference_server, model_load_policy, on_demand),
    %% Use the stub backend so the synthetic GGUF blob is loadable.
    application:set_env(barrel_inference_server, model_backend, barrel_inference_model_stub),
    application:set_env(barrel_inference_server, max_context_size, 1024),
...
end_per_suite(Config) ->
    ...
    application:unset_env(barrel_inference_server, model_backend),
    application:unset_env(barrel_inference_server, max_context_size),
    application:unset_env(barrel_inference_server, keep_alive_default_ms),
...
cli_full_lifecycle(Cfg) ->
    ...
    ?assertMatch({ok, _}, barrel_inference_server_models:get(<<"synthetic">>)),
    ...
    {ok, Alias} = barrel_inference_server_models:get(<<"alias-cli:v1">>),
    ...
    ?assertEqual({error, not_found}, barrel_inference_server_models:get(<<"synthetic">>)),
    ?assertEqual({error, not_found}, barrel_inference_server_models:get(<<"alias-cli:v1">>)).

%% CLI `run` against a stub-backed model: pull then run, expect non-empty output.
cli_run_streams_against_stub(Cfg) ->
    Spec = file_spec(Cfg),
    {ok, _} = barrel_inference_server_models:pull(Spec, #{
        name => <<"runtest">>, tag => <<"latest">>
    }),
...
%% {noproc, {barrel_inference_model, not_found, _}} because the loader was
...
run_cli(Cfg, Args) ->
    ...
    Env = "BARREL_INFERENCE_HOST=http://127.0.0.1:" ++ integer_to_list(Port),
    Cmd =
        Env ++ " " ++ Escript ++ " " ++
...
locate_escript() ->
    Root = walk_up(),
    Bin = filename:join([Root, "_build", "default", "bin", "barrel-inference"]),
...
make_tmp_dir() ->
    ...
    Dir = filename:join(
        Base,
        "barrel_inference_server_e2e_cli_" ++ integer_to_list(erlang:unique_integer([positive]))
    ),
