...
This file is the judgment for using them well. The tools talk to the barrel
daemon (default `http://localhost:8080`).

(This is the same operating guidance the Claude Code plugin ships as
`plugin/barrel-inference/skills/operating-barrel/SKILL.md`. Keep the two in sync.)
...
- `lazy`: pages fault in on first touch. Low startup, but every decode can stall
  on disk faults.
...
model's `num_ctx`. It does not silently truncate. To raise the window:
`barrel_edit_model num_ctx=<bigger>` (capped by the daemon's `max_context_size`),
then retry.
...
fresh `session_id` for unrelated work. Keep `num_seq_max` at least as large as
the number of concurrent sessions, or concurrent requests can deadlock.
